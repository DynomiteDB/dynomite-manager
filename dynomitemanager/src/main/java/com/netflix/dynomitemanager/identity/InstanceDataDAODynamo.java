...
public class InstanceDataDAODynamo
{
    ...
    @Inject
    public InstanceDataDAODynamo(IConfiguration config,HostSupplier hostSupplier) throws ConnectionException
    {
        this.config = config;
        ...
        if(KS_NAME == null || KS_NAME.isEmpty())
            throw new RuntimeException("Dynamo Keyspace can not be blank. Please use getDynamoKeyspaceName() property.");

        thriftPortForAstyanax = config.getDynamoThriftPortForAstyanax();
        if(thriftPortForAstyanax <= 0)
            throw new RuntimeException("Thrift Port for Astyanax can not be blank. Please use getDynamoThriftPortForAstyanax() property.");
	...
	public Set<AppsInstance> getAllInstances(String app)
	{
		...
		try {
			...
			ScanSpec scanSpec = new ScanSpec()
                    .withFilterExpression(CN_APPID + " = :" + CN_APPID)
                    .withValueMap(new ValueMap().withString(":" + CN_APPID, appId));
	...
	private AppsInstance transform(Item item)
	{
		...
		ins.setApp(item.get(CN_APPID));
		ins.setZone(item.get(CN_AZ));
		ins.setHost(item.get(CN_HOSTNAME));
		ins.setHostIP(item.get(CN_EIP));
		ins.setId(Integer.parseInt(item.get(CN_ID)));
		ins.setInstanceId(item.get(CN_INSTANCEID));
		ins.setDatacenter(item.get(CN_LOCATION));
		ins.setRack(item.get(CN_DC));
		ins.setToken(item.get(CN_TOKEN));
    ...
    private Supplier<List<Host>> getSupplier() {
        ...
        return new Supplier<List<Host>>() {
            ...
            public List<Host> get() {
                ...
                List<String> dynHostnames = new ArrayList<String>(Arrays.asList(StringUtils.split(config.getCommaSeparatedDynamoHostNames(), ",")));

                if(dynHostnames.size() == 0)
                    throw new RuntimeException("Dynamo Host Names can not be blank. At least one host is needed. Please use getCommaSeparatedDynamoHostNames() property.");
